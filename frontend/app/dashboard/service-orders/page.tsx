  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        ...
        actions={
          <>
            <Button variant="secondary" onClick={() => setModal({ type: "types" })}>
              <Tags size={16} /> Types
            </Button>
      ...
      {serviceTypes.length === 0 && !fetching && (
        <p className="text-sm text-ink-500">
          Add your first service type under &ldquo;Types&rdquo; above before creating an order.
        </p>
      )}
      ...
      {modal.type === "catalog" && (
        <Modal title="Service catalog" onClose={() => setModal({ type: "none" })}>
      ...
      {modal.type === "types" && (
        <Modal title="Service types" onClose={() => setModal({ type: "none" })}>
          <ServiceTypeManager
            types={serviceTypes}
