...
public class PaystackService {
     ...
     * Inline JS popup needs to open checkout without ever leaving the Billing page.
     *
     ...
     */
    public InitResult initializeTransaction(String email, long amountMinorUnits, String reference, Map<String, Object> metadata) {
        if (secretKey == null || secretKey.isBlank()) {
            throw new ApiException(HttpStatus.INTERNAL_SERVER_ERROR, "Paystack isn't configured on this server.");
        }
    ...
    /**
     * HMAC-SHA512 of the raw request body against the secret key, compared to
     * Paystack's x-paystack-signature header — per Paystack's webhook docs.
     * Must run against the untouched raw body bytes, before any JSON parsing.
     */
